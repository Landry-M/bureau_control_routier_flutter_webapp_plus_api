...
try {
    if ($permisId > 0) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM permis_temporaire WHERE cible_type = 'particulier' AND cible_id = :cible_id AND numero = :numero");
        $stmt->bindParam(':cible_id', $particulierId, PDO::PARAM_INT);
        $stmt->bindParam(':numero', $numero);
        $stmt->execute();
        $permis = $stmt->fetch(PDO::FETCH_ASSOC);
    } elseif ($particulierId > 0) {
        // Sans numéro fourni: tenter de récupérer le permis actif, sinon le plus récent
        $stmt = $pdo->prepare("SELECT * FROM permis_temporaire WHERE cible_type = 'particulier' AND cible_id = :cible_id AND statut = 'actif' ORDER BY id DESC LIMIT 1");
        $stmt->bindParam(':cible_id', $particulierId, PDO::PARAM_INT);
        $stmt->execute();
        $permis = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$permis) {
            // Si pas de permis actif, prendre le plus récent
            $stmt = $pdo->prepare("SELECT * FROM permis_temporaire WHERE cible_type = 'particulier' AND cible_id = :cible_id ORDER BY id DESC LIMIT 1");
            $stmt->bindParam(':cible_id', $particulierId, PDO::PARAM_INT);
            $stmt->execute();
            $permis = $stmt->fetch(PDO::FETCH_ASSOC);
        }
    ...
    if (!$permis) {
        ...
        $stmt->bindParam(':id', $particulierId, PDO::PARAM_INT);
        $stmt->execute();
...
$prenom = htmlspecialchars((string)($particulier['prenom'] ?? ''), ENT_QUOTES);
